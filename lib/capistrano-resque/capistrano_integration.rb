    ...
    def self.load_into(capistrano_config)
      capistrano_config.load do
        ...
        def start_command(queue, pid)
          "cd #{current_path} && RAILS_ENV=#{rails_env} QUEUE='#{queue}' PIDFILE=#{pid} BACKGROUND=yes VERBOSE=1 INTERVAL=#{interval} #{fetch(:bundle_cmd, "bundle")} exec rake environment resque:work"
        end

        def stop_command
          "if [ -e #{previous_release}/tmp/pids/resque_work_1.pid ]; then \
           for f in `ls #{previous_release}/tmp/pids/resque_work*.pid`; \
        ...
        def start_scheduler(pid)
          "cd #{current_path} && RAILS_ENV=#{rails_env} \
           ...
           #{fetch(:bundle_cmd, "bundle")} exec rake environment resque:scheduler"
        end
